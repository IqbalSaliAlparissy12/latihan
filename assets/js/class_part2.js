...
class Book {
  ...
  static setTableRow() {
    List_OF_BOOK.forEach((data) => {
      $("#table-book tbody").append(
        `<tr><td>${data.title}</td><td>${data.author}</td><td>${data.year}</td><td>${
  ...
  static find(title) {
    ...
    if (title) {
      ...
      if (book) {
        $("#table-book tbody").append(
          `<tr><td>${book.title}</td><td>${book.author}</td><td>${book.year}</td><td>${
...
const novel_3 = new NovelBook(
  "Cantik Itu Kamu",
  "Iqbal Sali Alparissy",
  "2002"
);
...
const biografi_2 = new BiografiBook("Thariq Khulafa", "Salaludin", "1990");
